...
## Round 30: Triton Flash Attention 实现 (2026-03-19)

Triton 实现：~100 行 Python，使用 tl.dot 自动利用 Tensor Core。

**正确性**: 全部通过，max_diff 与 CUDA kernel 相同 (0.000122~0.000244)

**性能对比**:

| Config | PyTorch SDPA | Triton | Custom CUDA |
|--------|-------------|--------|-------------|
| 1,8,512,512,64 | 0.057ms | 0.066ms (1.16x) | 0.216ms (3.8x) |
| 1,8,1024,1024,64 | 0.174ms | 0.163ms (0.94x) | 0.812ms (4.7x) |
| 1,16,2048,2048,64 | 0.727ms | 0.695ms (0.96x) | 4.419ms (6.1x) |
| 2,16,2048,2048,64 | 1.581ms | 1.331ms (0.84x) | 8.759ms (5.5x) |
| 4,16,4096,4096,64 | 13.148ms | 11.044ms (0.84x) | 68.433ms (5.2x) |

**结论**: Triton 在大配置上甚至超越 PyTorch SDPA，用 ~100 行 Python 达到了手写 CUDA 300 行无法企及的性能。
